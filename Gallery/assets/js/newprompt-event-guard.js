(function(){
  ...
  async function doSave(e){
    try{
      ...
      // NOTE: tags are no longer read or saved
      ...
      if (!title){
        ...
        console.warn("[NPV] Title empty");
        if (titleEl){
          titleEl.classList.add("input-error");
          titleEl.focus();
          titleEl.scrollIntoView({block:"center"});
          setTimeout(()=> titleEl.classList.remove("input-error"), 1200);
        }
        return;
      }
      ...
      // tags.json — write TITLE ONLY (no tags field)
      {
  ...
  function wireOnce(){
    ...
    if (!document.__npv_guard){
      ...
      // Submit inside the New Prompt virtual page
